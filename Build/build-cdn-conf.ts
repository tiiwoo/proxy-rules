...
import {
  fetchRemoteTextAndReadByLine,
  readFileByLine,
} from './lib/fetch-text-by-line';
import { createTrie } from './lib/trie';
import { task } from './lib/trace-runner';
import { processLine } from './lib/process-line';
import { SHARED_DESCRIPTION } from './lib/constants';

const publicSuffixPath: string = path.resolve(
  import.meta.dir,
  '../node_modules/.cache/public_suffix_list_dat.txt'
);
...
const getS3OSSDomains = async (): Promise<Set<string>> => {
  ...
  if (await publicSuffixFile.exists()) {
    ...
    console.log(
      'public_suffix_list.dat not found, fetch directly from remote.'
    );
    for await (const line of await fetchRemoteTextAndReadByLine(
...
const buildCdnConf = task(import.meta.path, async () => {
  ...
  for await (const l of readFileByLine(
    path.resolve(import.meta.dir, '../Source/non_ip/cdn.conf')
  )) {
  ...
  (await getS3OSSDomainsPromise).forEach((domain: string) => {
