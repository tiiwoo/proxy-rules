...
const validateDomainSet = async (filePath) => {
  ...
  for (let i = 0, len = domainSetLines.length; i < len; i++) {
    ...
    if (
      (parsed.isPrivate || parsed.isIcann) &&
      domain === parsed.publicSuffix
    ) {
      console.error(
        `[${filePath}]`,
        picocolors.yellow(domain),
        picocolors.red('is in public suffix list!')
      );
    }
  }
};
  ...
  for (let i = 0, len = rulesetLines.length; i < len; i++) {
    ...
    if (domain !== parsed.publicSuffix) {
      if (!SPECIAL_SUFFIXES.has(domain)) {
        console.warn(
          picocolors.yellow(domain),
          picocolors.green('is not in public suffix list!')
        );
      }
    }
  }
...
(async () => {
  ...
  await Promise.all(domainsetFiles.map((file) => validateDomainSet(file)));
  // await Promise.all(
